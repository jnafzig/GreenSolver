function testgreensintegral( testCase )
    %TEST test that the greens integral function correctly integrates the
    % greens function
    x = [-1;0;1];
    ...
    Check = greensintegral(x,kval,sol);

    Check1 = integral2(@(x1,x2) greens(x1,x2,x,kval,sol),...
                x(1),x(2),x(1),@(x) x,...
                'RelTol',eps,...
                'AbsTol',eps) ...
            + integral2(@(x1,x2) greens(x1,x2,x,kval,sol),...
                x(1),x(2),@(x) x,x(2),...
                'RelTol',eps,...
                'AbsTol',eps);

    Check2 = integral2(@(x1,x2) greens(x1,x2,x,kval,sol),...
                    x(1),x(2),x(2),x(3),...
                    'RelTol',eps,...
                    'AbsTol',eps);

    testCase.verifyEqual(abs(Check1-Check(2,2)),0,'AbsTol',1e2*eps);
    testCase.verifyEqual(abs(Check2-Check(3,2)),0,'AbsTol',1e2*eps);
